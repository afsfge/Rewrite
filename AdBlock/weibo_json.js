...
const mainConfig = {
    ...
    modifyStatusesUrls = ["statuses/friends/timeline", "statuses/unread_friends_timeline",
        "statuses/unread_hot_timeline", "groups/timeline", "statuses/friends_timeline"],
    otherUrls = {
...
function getModifyMethod(e) {
    ...
    for (let [i, n] of Object.entries(otherUrls))
        if (e.indexOf(i) > -1) return n;
    return null
}

function removeIntlOpenAds(e) {
    return e.data && 0 !== e.data.length && (e.data.ad_list = [], e.data.gdt_video_ad_ios = [], e.data.display_ad = 0,
        e.data.ad_ios_id = null, e.data.app_ad_ios_id = null, e.data.reserve_ad_ios_id = "", e.data.reserve_app_ad_ios_id =
        "", e.data.ad_duration = 604800, e.data.ad_cd_interval = 604800, e.data.pic_ad = []), e
}
...
function modifiedUserCenter(e) {
    return e.data && 0 !== e.data.length && e.data.cards && (e.data.cards = Object.values(e.data.cards).filter(e =>
        "personal_vip" != e.items[0].type)), e
}

function removeTopics(e) {
    return e.data && (e.data.topics && delete e.data.topics, e.data.discover && delete e.data.discover), e
}

function isAd(e) {
    return !!e && !!("广告" == e.mblogtypename || "热推" == e.mblogtypename || e.promotion ? .type == "ad" || e.page_info ?
        .actionlog ? .source == "ad" || e.content_auth_info ? .content_auth_title == "广告" || e.common_struct && e.common_struct[
            0] ? .actionlog ? .source ? .includes("ad"))
}
...
function removeMainTab(e) {
    ...
    let t = [];
    for (let o of e.items) isAd(o.data) || (o.data ? .common_struct && delete o.data.common_struct, o.category ?
        "group" != o.category && t.push(o) : t.push(o));
...
function removeMain(e) {
    ...
    for (let o of e.items)
        ...
        if (o.items.length > 0 && o.items[0].data ? .itemid ? .includes("search_input")) o.items = o.items.filter(e =>
            e ? .data ? .itemid ? .includes("mine_topics") || e ? .data ? .itemid ? .includes("search_input")), o.items[
            0].data.hotwords = [{
            word: "搜索超话",
            tip: ""
        }], t.push(o);
            ...
            o.items.length > 0 ? o.items = Object.values(o.items).filter(e => "feed" == e.category) : t.push(o)
        }
    ...
    for (let i of t) {
        ...
        if (i.mblog) {
            ...
            if ("bottom_mix_activity" == i.itemid) n = !1;
                ...
                let r = i.card_group;
                if (!r) continue;
                if (["guess_like_title", "cats_top_title", "chaohua_home_readpost_samecity_title"].indexOf(r[0].itemid) >
                    -1) n = !1;
                else if (r.length > 1) {
                    let d = [];
                    for (let s of r) - 1 == ["chaohua_discovery_banner_1", "bottom_mix_activity"].indexOf(s.itemid) &&
                        d.push(s);
                    i.card_group = d
                }
            }
...
function checkSearchWindow(e) {
    return !!mainConfig.removeSearchWindow && "card" == e.category && (e.data ? .itemid == "finder_window" || e.data ?
        .itemid == "more_frame" || e.data ? .card_type == 208 || e.data ? .card_type == 217 || e.data ? .card_type ==
        19 || e.data ? .mblog ? .page_info ? .actionlog ? .source ? .includes("ad"))
}
...
function removeSearch(e) {
    ...
    for (let o of e.items)
        ...
        } return e.items = t, e.loadedInfo && (e.loadedInfo.searchBarContent = [], e.loadedInfo.headerBack && (e.loadedInfo
        .headerBack.channelStyleMap = {})), log("remove_search success"), e
}
...
function removePage(e) {
    return removeCards(e), mainConfig.removePinedTrending && e.cards && e.cards.length > 0 && e.cards[0].card_group &&
        (e.cards[0].card_group = e.cards[0].card_group.filter(e => !(e ? .actionlog ? .ext ? .includes("ads_word") || e ?
            .itemid ? .includes("t:51") || e ? .itemid ? .includes("ads_word")))), e
}
...
function removeCards(e) {
    ...
    for (let o of e.cards) {
        ...
        if (i && i.length > 0) {
            let n = [];
            for (let a of i) 118 == a.card_type || isAd(a.mblog) || -1 != JSON.stringify(a).indexOf("res_from:ads") ||
                n.push(a);
...
function removeTimeLine(e) {
    ...
    let o = [];
    for (let i of e.statuses) isAd(i) || (lvZhouHandler(i), i.common_struct && delete i.common_struct, i.category ?
        "group" != i.category && o.push(i) : o.push(i));
    e.statuses = o
}
...
function removeVideoRemind(e) {
    e.bubble_dismiss_time = 0, e.exist_remind = !1, e.image_dismiss_time = 0, e.image = "", e.tag_image_english = "", e
        .tag_image_english_dark = "", e.tag_image_normal = "", e.tag_image_normal_dark = ""
}
...
function itemExtendHandler(e) {
    ...
    mainConfig.removeFollow && e.follow_data && (e.follow_data = null), mainConfig.removeRewardItem && e.reward_info &&
        (e.reward_info = null), e.page_alerts && (e.page_alerts = null);
    try {
    ...
    try {
        for (let t of e.items)
            if ("mainnums_friends" === t.itemId) {
                ...
                t.click.modules[0].scheme = o.replace("231093_-_selfrecomm", "231093_-_selffollowed"), log(
                    "updateFollowOrder success");
                return
            }
    } catch (i) {
...
function updateProfileSkin(e, t) {
    try {
        ...
        for (let n of e.items)
            if (n.image) try {
                dm = n.image.style.darkMode, "alpha" != dm && (n.image.style.darkMode = "alpha"), n.image.iconUrl =
                    o[i++], n.dot && (n.dot = [])
            } catch (a) {}
...
function removeHome(e) {
    ...
    for (let o of e.items) {
        let i = o.itemId;
        if ("profileme_mine" == i) mainConfig.removeHomeVip && (o = removeHomeVip(o)), o.header ? .vipIcon && delete o.header
            .vipIcon, updateFollowOrder(o), t.push(o);
        else if ("100505_-_top8" == i) updateProfileSkin(o, "profileSkin1"), t.push(o);
        else if ("100505_-_newcreator" == i) "grid" == o.type ? (updateProfileSkin(o, "profileSkin2"), t.push(o)) :
            mainConfig.removeHomeCreatorTask || t.push(o);
        else {
            if (["mine_attent_title", "100505_-_meattent_pic", "100505_-_newusertask", "100505_-_vipkaitong",
                    "100505_-_hongbao2022", "100505_-_adphoto", "100505_-_advideo"].indexOf(i) > -1 || i.match(
                    /100505_-_meattent_-_\d+/)) continue;
            t.push(o)
        }
    }
    return e.items = t, e
...
function removeComments(e) {
    ...
    let i = [];
    for (let n of o) {
        let a = n.adType || ""; - 1 == t.indexOf(a) && 6 != n.type && i.push(n)
    }
...
function containerHandler(e) {
    mainConfig.removeInterestFriendInTopic && "超话里的好友" === e.card_type_name && (log("remove 超话里的好友"), e.card_group = []),
        mainConfig.removeInterestTopic && e.itemid && (e.itemid.indexOf("infeed_may_interest_in") > -1 ? (log(
            "remove 感兴趣的超话"), e.card_group = []) : e.itemid.indexOf("infeed_friends_recommend") > -1 && (log(
            "remove 超话好友关注"), e.card_group = []))
}
...
function removePhpScreenAds(e) {
    if (!e.ads) return e;
    for (let t of (e.show_push_splash_ad = !1, e.background_delay_display_time = 0, e.lastAdShow_delay_display_time = 0,
            e.realtime_ad_video_stall_time = 0, e.realtime_ad_timeout_duration = 0, e.ads)) t.displaytime = 0, t.displayintervel =
        86400, t.allowdaydisplaynum = 0, t.displaynum = 0, t.displaytime = 1, t.begintime = "2029-12-30 00:00:00", t.endtime =
        "2029-12-30 23:59:59";
    return e
}
